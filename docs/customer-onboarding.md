...
3. **Request access** — to connect to the iCap SEO service, the customer requests a registration token from
   iCapSolutions (see [Requesting a token](#requesting-a-registration-token) below).
...
Registration tokens are issued by iCapSolutions on request — this is not yet a self-service signup. Request one at
[icapsolutions.com](https://www.icapsolutions.com/services/wordpress-seo-plugin.html) or from your account
contact, then follow [Getting started](getting-started.md) to connect and register your site.
